...
include("importmsh_phasefield force.jl") 
elements,nodes = import_fem("./msh/inclined_interface2_force.msh")
nₚ = length(nodes)
...
for n in 1:total_steps
    ...
    prescribe!(elements["Γᵗ"],:t₂=>(x,y,z)->F/20)
    prescribe!(elements["Γᵗ"],:t₁=>(x,y,z)->0.0)
    ops[7](elements["Γᵗ"],fext)
    iter = 0
    ...
    while normΔ > tol && iter < 10
        ...
        while normΔd > tol && iter₂ < 10
            ...
            if iter₂ == 1
               ...
               solve!(ps,Δd,k+kᵅ₁,fᵅ₁+fext-fint)
            else
               # Δd .= (k+kᵅ₁+kᵅ₂)\(-fint)
               solve!(ps,Δd,k+kᵅ₁,fext-fint)
            end
    ...
    fo = open("./vtk/friction_force/figure"*string(n,pad=4)*".vtk","w")
